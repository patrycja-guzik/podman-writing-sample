...
\item  Create a configuration for the mariadb database container. Save it as
  the \mdinlinecode{mariadb.conf}.
\begin{mdcodeblock}
{
    "image" : "wordpress",
    "env": {\begin{mdblockquote}
...
http://d/v3.0.0/libpod/pods/my-pod/json |jq
\end{mdcodeblock}
\textbf{Expected output:} JSON structure with all the information describing a pod, such as a name, creation timestamp, number of containers, state of containers, etc.
\end{mdenumerate}
\hypertarget{additional-resources}{%
\textbf{Additional resources}\label{additional-resources}}
\begin{mditemize}
...
\textbf{Procedure}
\begin{mdenumerate}
 \item Stop \mdinlinecode{my-pod} pod using REST API.
 \begin{mdcodeblock}
$ curl -XPOST --unix-socket /run/user/${UID}/podman/podman.sock \
-H content-type:application/json \
